...
4. **Cài đặt xác thực thật**: `AuthServiceImpl.authenticateCredentials()` đang ném 401 cho mọi request (fail-closed). Thay bằng kiểm tra thông tin đăng nhập thật và trả về danh sách vai trò của người dùng.
...
- **Bố cục package theo concern**: `config/` (mỗi concern một subpackage), `controller/vN`, `service` + `service/impl`, `dto/app/{request,response}`, `enums`, `exception`, `filter`, `security`, `client`, `context`, `converter`, `utils`.
...
- **MySQL — JPA (Hibernate) + JdbcTemplate cùng lúc** (`spring-boot-starter-data-jpa` + `mysql-connector-j`): TẮT bằng cách LOẠI `DataSourceAutoConfiguration` (`spring.autoconfigure.exclude`, env `MYSQL_AUTOCONFIGURE_EXCLUDE`) → JdbcTemplate/JPA/repository tự backs-off. BẬT: để env `MYSQL_AUTOCONFIGURE_EXCLUDE=` rỗng + creds + `MYSQL_ENABLED=true`. Boot 4 FQN: `org.springframework.boot.jdbc.autoconfigure.DataSourceAutoConfiguration`. Health: `management.health.db.enabled`.
...
- Chỉ một `application.yaml` ở gốc: giữ tên app + import thư mục profile + khối **Spring Cloud Consul**. Cấu hình theo môi trường nằm ở `resources/profiles/<profile>/` tách nhỏ theo concern: `server.yaml`, `web.yaml`, `security.yaml`, `redis.yaml`, `http-client.yaml`, `cache.yaml`, `observability.yaml`, `resilience.yaml`, và mỗi infra bên thứ 3 một file: `mysql.yaml`, `mongodb.yaml`, `rabbitmq.yaml`, `kafka.yaml` (tất cả OPT-IN, mặc định TẮT — xem mục B-infra).
...
- Spring KHÔNG hỗ trợ wildcard cho `spring.config.import` trên classpath. Để nạp các file mà không liệt kê, `spring.config.name` = `application,server,web,security,redis,http-client,cache,observability,resilience,mysql,mongodb,rabbitmq,kafka` được đặt ở **hai nơi phải đồng bộ**:
