...
## Opt-in infra (all default-off)

Redis (cache + Redisson lock), Consul (config + discovery), MongoDB, RabbitMQ, Kafka — one yaml per profile (Consul's block lives in the root `application.yaml`, bootstrap-time), native Spring properties, env-gated (`REDIS_ENABLED`, `CONSUL_ENABLED` + `CONSUL_DISCOVERY_ENABLED`, `MONGODB_ENABLED`, `RABBITMQ_ENABLED`, `KAFKA_ENABLED`). The base builds/tests/runs with none installed; enabled paths are Testcontainers-verified and self-skip without Docker, so `./mvnw test` stays green. **Before touching any of these, read AGENTS.md §B (Redis) / §B-infra — they hold the usage rules and the Boot 4 migration gotchas (Mongo prefix move, `spring-boot-kafka` module, `DataRedisProperties`) that fail silently if missed.** Per-cache TTLs come from the `KeyCache` enum (currently `ROLE_PERMISSIONS`/`ROLE_MENUS`, 6h, for IAM). MySQL is vestigial — replaced by Oracle (see above); `mysql` stays harmlessly in `spring.config.name`.
